...
public class RGBBlock extends Block implements BlockEntityProvider {
	...
	public void onPlaced(World world, BlockPos pos, BlockState blockState, LivingEntity entity, ItemStack item) {
		...
		if (blockEntity instanceof RGBBlockEntity) {
			...
			((RGBBlockEntity)blockEntity).setColor(color);
		}
	...
	private static ItemStack ApplyNBT(@Nullable BlockEntity blockEntity, ItemStack stack) {
		stack.getOrCreateSubNbt("display").putInt("color", ((RGBBlockEntity)blockEntity).getColor());
		return stack;
	}
	...
	public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
		...
		stack.getOrCreateSubNbt("display").putInt("color", ((RGBBlockEntity)world.getBlockEntity(pos)).getColor());
